...
public class TradeJobSchedulerTest extends TradeTransmissionApplicationTests{
	...
	public void testExpireSchedulesWhenAvailable() {
		...
		Mockito.when(tradeDao
				.findByExpiredAndMaturityDateLessThan(ApplicationConstant.TRADE_EXPIRED_N, LocalDate.now())).thenReturn(tradeDetails);
		
		Mockito.when(tradeDao.saveAll(tradeDetails)).thenReturn(tradeDetails);
		
		try {
			tradeJobScheduler.tradeExpireSchedules();
		}catch(Exception exp) {
			expected = exp;
		}
