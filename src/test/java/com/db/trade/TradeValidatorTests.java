...
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
	public class TradeValidatorTests extends TradeTransmissionApplicationTests{
		...
		public void testtradePreValidationInvalidMaturityDate() {
			...
			try {
				...
				Assert.assertEquals(ExceptionConstants.ERROR_104, be.getErrorCode());
			}
		}
		...
		public void testvalidateTradeVersionWithSameVersion() {
			Trade  trade = new Trade("T1", 1l,"CP-1", "B1",LocalDate.now().plusDays(1L),ApplicationConstant.TRADE_EXPIRED_N);
			TradeDto tradeRequest = new TradeDto("T1", 1, "CP-1", "B1", LocalDate.now().plusDays(1L));
			Exception ex = null;
			try {
				Mockito.when(tradeDao.findTradeDetailsByOrderAndLimit(tradeRequest.getTradeId())).thenReturn(trade);
		...
		public void testvalidateTradeVersionWithFewerVersion() {
			Trade  trade = new Trade("T1", 2l,"CP-1", "B1",LocalDate.now().plusDays(1L),ApplicationConstant.TRADE_EXPIRED_N);
			TradeDto tradeRequest = new TradeDto("T1", 1, "CP-1", "B1", LocalDate.now().plusDays(1L));
			Exception ex = null;
			try {
				Mockito.when(tradeDao.findTradeDetailsByOrderAndLimit(tradeRequest.getTradeId())).thenReturn(trade);
